...
public:
    GLActiveTextureGuard() { glGetIntegerv(GL_ACTIVE_TEXTURE, &prevActiveTex); }
    ~GLActiveTextureGuard() { glActiveTexture(prevActiveTex); }
...
DrawableCustom::DrawableCustom(std::string name_)
...
void mbgl::style::CustomDrawableTweaker::execute(gfx::Drawable& drawable, PaintParameters& parameters) {
    ...
    tile_matrix = LayerTweaker::getTileMatrix(
        tileID, parameters, {{0, 0}}, style::TranslateAnchorType::Viewport, true, true, drawable, false);
}
