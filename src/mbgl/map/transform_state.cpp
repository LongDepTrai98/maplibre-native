...
void TransformState::getworldToCameraMatrix(mat4& matrix) const 
{
    matrix = camera.getWorldToCamera(scale, viewportMode == ViewportMode::FlippedY);
}

void TransformState::cameraToClipMatrix(mat4& matrix, uint16_t nearZ, bool aligned) const 
{
    if (size.isEmpty()) {
        return;
    }
    ...
    mat4 worldToCamera = camera.getWorldToCamera(scale, viewportMode == ViewportMode::FlippedY);
    matrix = camera.getCameraToClipPerspective(
        getFieldOfView(), static_cast<double>(size.width) / size.height, nearZ, farZ);
}
